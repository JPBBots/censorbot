module.exports = (client) => {
  ...
  client.q_censored = async (mem) => {
    ...
    let c = [true, false]
    const k = await client.db.find(guild.id, 'censor')
    if (k == false) c = [false, true]
    const r = await client.db.find(guild.id, 'role')
    if (!r) {
  ...
  client.q_log = async (g) => {
    const glog = await client.db.find(g.id, 'log')
  ...
  Boolean.prototype.swap = function () { return this != true }
  ...
  client.sendErr = async (msg, errormessage) => {
    const o = client.u.embed
      .setColor('DARK_RED')
      .setTitle(errormessage)
      .setFooter('Command Ran by ' + msg.author.username)
    const me = await msg.channel.send(o)
    setTimeout(() => {
      me.delete()
    }, 4000)
    return true
  ...
  // filter test
  client.test = (s, g) => {
    const a = client.RemoveAccents(s).slice().trim().split(/ +/g)
    const u = require('./filter/filter.js')(client, a, g, true)
    return u
  }
  // json db control
  ...
  client.update_count = () => {
    client.shard.fetchClientValues('guilds.size').then(results => {
      const aa = results.reduce((prev, guildCount) => prev + guildCount, 0)
