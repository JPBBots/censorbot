...
const settings = {
    ...
    method: {
        ...
        msg: async(message, arg, db, client) => {
            var a = (await db.get("censor")).msg ? false : true
            sendSettings(client, message, {
                data: {
        ...
        emsg: async(message, arg, db, client) => {
            var a = (await db.get("censor")).emsg ? false : true
            sendSettings(client, message, {
                data: {
        ...
        nick: async(message, arg, db, client) => {
            var a = (await db.get("censor")).nick ? false : true
            sendSettings(client, message, {
                data: {
        ...
        react: async(message, arg, db, client) => {
            var a = (await db.get("censor")).react ? false : true
            sendSettings(client, message, {
                data: {
    ...
    base: {
        ...
        toggle: async(message, arg, db, client) => {
            ...
            sendSettings(client, message, {
                ...
                db.set("base", now);
                if(now) sendSettings(client, message, {
                    data: {
                        setting: settings["filter"].desc.name,
                        then: "Cleared",
                        now: "all"
