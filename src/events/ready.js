...
module.exports = async(client) => {
  ...
  require(client.mappings.presences)(client)

  setInterval(async() => {
    client.setPresence()
  }, 1800000)
  client.setPresence = async() => {
    client.user.setPresence(await client.presences[client.p]())
  }

  client.shard.broadcastEval("true")
    .then(() => {
      client.setPresence()
    })
    .catch((e) => {})
  if (client.shard.id == client.shard.count - 1) {
    client.shard.fetchClientValues('guilds.size').then(results => {
      ...
      client.shard.broadcastEval(`this.setPresence()`)
  ...
  if (client.shard.id == 0) {
    setInterval(async() => {
      const vv = await client.shard.fetchClientValues('guilds.size')
      dbl.postStats(vv)
      client.shard.fetchClientValues('guilds.size').then(results => {
  ...
  if (fs.existsSync('./r.json')) {
    const o = JSON.parse(fs.readFileSync('./r.json'))
    if (o) {
      if (o.shard != client.shard.id) return
      const Discord = require('discord.js')
      const a = await client.channels.get(o.chan).messages.fetch(o.mes)
