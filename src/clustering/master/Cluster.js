...
class Cluster extends EventEmitter {
   ...
   */
  setup () {
    this.thread = new Worker(resolve('../worker/service.js'), { workerData: { id: this.id, job: this.job, shards: this.shards, shardCount: this.master.shardCount } })
    ...
    if (this.master.spawned) this.spawn(this.master._gateway)
  }
  ...
  spawn (gateway) {
    return new Promise((resolve) => {
      this.send('SPAWN', { gateway, spawned: this.master.spawned, inactive: this.inactive })
